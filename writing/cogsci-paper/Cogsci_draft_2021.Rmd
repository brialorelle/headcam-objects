...
Importantly, we found that the distribution of the objects children were interacting with roughly followed a power law distribution, when we included separate categories for different formats  ($\alpha =$ `r round(pl_fit$alpha,2)`, Kolmogorov-Smirnov distance $D=$ `r round(pl_fit$KS.stat, 2)`, $p=$ `r round(pl_fit$KS.p,2)`), when we collapsed across them ($\alpha =$ `r round(pl_fit_formats$alpha,2)`, $D=$ `r round(pl_fit_formats$KS.stat, 2)`, $p=$ `r round(pl_fit_formats$KS.p,2)`), or when we excluded *book*, *toy*, and *person* ($\alpha =$ `r round(pl_fit_limited$alpha,2)`, $D=$ `r round(pl_fit_limited$KS.stat, 2)`, $p=$ `r round(pl_fit_limited$KS.p,2)`). 
Since the K-S test $p$-values are not <.05, we cannot reject the null hypothesis that these data are power-law distributed.
Thus, overall these results confirm that the distribution of the objects that children interact with is highly skewed, generalizing the findings of Clerkin et al., 2017.
...
However, some word learning studies of adults have found that skewed frequency distributions can improve learning -- even in the Zipfian case [@hendrickson2019cross] -- and that online learning via semantic bootstrapping may explain the benefit of varying frequency [@kachergis2017bootstrapping].
