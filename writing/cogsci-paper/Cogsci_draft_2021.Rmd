...
get_avg_permutation_matrix <- function(temp, Nsim=100) {
  col_names = colnames(temp)
  ...
  for(i in 1:Nsim) {
    ...
    perm_coocs = perm_coocs + get_img_category_coocs(permed)
...
perm_coocs <- get_avg_permutation_matrix(temp)

perm_vals = unlist(perm_coocs)
perm_vals = perm_vals[which(perm_vals > 0)] # 0s are from the diagonal
summary(perm_vals) # min: 583.7, max: 595.9
```
