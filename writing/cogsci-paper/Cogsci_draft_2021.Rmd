...
While some frequent activity context activities (i.e. mealtime) may have a fixed set of categories that are usually co-present (i.e. utensils/dishes and food), others may not be so easy to intuit. Figure \ref{fig:coocc_stats} shows the co-occurrence of the broad categories that were annotated.
First, it is apparent that people appear quite often, and in scenes with many of the other object categories (in particular, with clothing, books, utensils, and food).
Furniture is another promiscuous category, although sensibly not often appearing with plants or real vehicles.
Food and utensils are distinctively frequent neighbors, likely identifying mealtime.
Books often appear with toy animals and people, but otherwise there is no distinctive profile for bookreading.
Plants often appear with "big objects" and real vehicles, likely identifying outdoor contexts, which future research will have to further elaborate.
<!-- GK: hard to pick out anything surprising here...-->
...
```{r coocc_stats, echo=F, include = T, fig.env = "figure", fig.pos = "h", fig.align='center', fig.width=3.5, fig.height=3.5, set.cap.width=T, num.cols.cap=1, fig.align = "center", fig.cap = "Co-ccurrence between different categories detected in the dataset across all frames. Lighter values indicate higher probabilities of co-occurrence (max=.8, min=0)."}
...
# General Discussion
Here, we analyzed the consitency and variablity of the categories and the objects in the infant view, examining a sample of random frames taken from a longitudinal dataset of two children [@SAYcam]. 
Overall, we found relative consistency in children's visual environment over development, in contrast to prior work on the prevalence of social signals over this same developmental time period. 
...
While the proportion of small objects in view did seem to show something of a U-shaped curve around the first birthday -- consistent with the hypothesis that children may see fewer toys as they are learning to crawl and walk [@long2020] -- this remains to be confirmed with future research. Instead, postural developments and cognitive milestones may lead to finer-grained changes in how children view objects or the diversity of exemplars that tend to be view.
...
More broadly, this work also highlights the need for systematic investigations in how the frequency of categories in the child's view interacts with different attentional biases, learning mechanisms, and social cues to produce robust representations that support category and early language learning. 
